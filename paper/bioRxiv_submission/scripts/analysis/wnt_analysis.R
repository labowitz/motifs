...
filename = "wnt" # how name is appended to files for pathway
...
min_expr_threshold = 0.3 # Wnt minimum expression threshold for gene to be on
...
g <- silhouette_zscore(silh_result = silh_plt,
                       ...
                       x_offset = 6,
                       max.y = 0.55,
                       min.y =-0.05 # adjust axis parameters 
)
g
...
pheatmap(sqrt(x), treeheight_row = 20,
         treeheight_col = 20,
         ...
         cluster_cols = F,
         cluster_rows = F,
         filename = paste(output_dir, filename, "_tissue_distribution.pdf", sep = ""),
...
control_res$diversity %>% 
  dplyr::filter(type=='transcriptome') %>% # choose the null model 
  pull(d) %>% quantile(0.30) -> divers_quantile
