...
pathway_df = read.csv("./data/raw_data/pathbank/pathway_df_zara.csv")
...
optimal_k_pathway = perc_k_finder(silh_z_plt[[2]], percentile =  0.9)
...
ggsave(paste(fig_dir, "Figure_4B_",pathway_name,".pdf",sep=""), width = 7, height = 7)

# ECDF of dispersion scores
ecdf_plot = ecdf_diversity(control_res)
ggsave(paste(fig_dir, "Figure_4C_",pathway_name,".pdf",sep=""), width = 7, height = 7)

# Motif heatmap, motifs defined as profiles above 90th percentile of dispersion
pdf(paste(fig_dir, "Figure_4D_",pathway_name,".pdf",sep=""))
...
ggsave(plot = motif_ct, paste(fig_dir, "Figure_4E_",pathway_name,".pdf",sep=""))

# Private profiles heatmap
pdf(paste(fig_dir, "Figure_S3D_",pathway_name,".pdf",sep=""))
...
ggsave(plot = g_umap, paste(fig_dir, "Figure_2C_",pathway_name,".pdf",sep=""))
...
counts_bar <- geneCounts(seurat_obj = master_seurat,
                         ...
                         min_genes_on = min_genes_pathway,
                         min_expr = min_expr_threshold)
ggsave(paste(fig_dir, "Figure_S2A_",pathway_name,".pdf",sep=""))
...
ggsave(plot = ecdf_thresh, filename = paste(fig_dir, "Figure_S2B_",pathway_name,".pdf",sep=""))
...
ggsave(plot = coexp_heat, filename = paste(fig_dir, "Figure_S2C_",pathway_name,".pdf",sep=""))
