...
pipe_run <- quickPipeline(master_seurat = master_seurat, 
                          ...
                          which_profiles = 'both', 
                          save_pdf = T, 
                          pdf_file = paste(fig_dir, "Figure_5_figure_supplement_2A.pdf", sep = ""))
...
pdf(paste(fig_dir, "Figure_5C_left.pdf", sep = ""))
...
pheatmap(sqrt(x), treeheight_row = 20,treeheight_col = 20,
         ...
         filename = paste(fig_dir, "Figure_5C_right.pdf", sep = ""),
...
# 5. Make heatmap of averaged profiles selected
pdf(paste(fig_dir, "Figure_5_figure_supplement_3A_right.pdf", sep = ""))
motif_heatmap <- superheat(diverse_mat[,genesPathway(pathway_name)],
                           pretty.order.rows = T,
                           heat.pal = black_pal(10),
                           bottom.label.text.angle = 90, 
